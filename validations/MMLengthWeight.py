...
class MMLengthWeight(QObject):
    ...
    def validate(self, currentValue,  measurements,  values):
        ...
        length = (float(values[measurements.index('fork_length_mm')])
        if "fork_length_mm" in measurements
        else (
            float(values[measurements.index('standard_length_mm')])
            if "standard_length_mm" in measurements
            else None
            )
        )
