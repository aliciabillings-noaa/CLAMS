...
"""
    :module:: LengthSexMaturity

    :synopsis: LengthSexMaturity is a conditional that checks if a body should be frozen
...
class LengthSexMaturity(QObject):
    ...
    def __init__(self, db, schema, speciesCode, parent=None):
        ...
        age1Length, = query.first()

        #  extract returned results
        self.age1Length=float(age1Length)
    ...
    def evaluate(self,   measurements,  values,  result):
        ...
        # figure out the rule
        if values[measurements.index('length')] is not None:
